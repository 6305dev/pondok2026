                    ...
                    <div class="border border-white/30 rounded-2xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 bg-white/40 backdrop-blur-md">
                        ...
                        >
                            ...
                            <svg
                                ...
                                :class="{ 'rotate-180 text-blue-700': isGroupOpen('{{ $groupSlug }}') }"
                        ...
                        >
                            @foreach($items as $index => $item)
                                ...
                                <div class="border border-white/20 rounded-xl overflow-hidden bg-white/30 transition-all duration-200">
                                    ...
                                    <button
                                        type="button"
                                        id="accordion-header-{{ $accordionSlug }}"
                                        aria-controls="accordion-content-{{ $accordionSlug }}"
                                        class="w-full flex justify-between items-center p-3 text-left hover:bg-white/40 focus:outline-none focus:ring-2 focus:ring-blue-600 transition-all duration-150"
                                    ...
                                    >
                                        <span class="flex items-center gap-2.5 text-xs md:text-sm font-bold text-gray-900">
                                            <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-3.5 h-3.5 text-blue-700">
                                        ...
                                        <svg
                                            ...
                                            :class="{ 'rotate-180 text-blue-700': isAccordionOpen('{{ $accordionSlug }}') }"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                        >
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"></path>
                                        </svg>
                                    ...
                                    <!-- Konten Persyaratan -->
                                    <div
                                        id="accordion-content-{{ $accordionSlug }}"
                                        role="region"
                                        aria-labelledby="accordion-header-{{ $accordionSlug }}"
                                                        ...
                                                        <li class="flex items-start gap-3 py-2 border-b border-white/20 last:border-b-0">
                                                            ...
                                                            <span class="text-gray-900 font-semibold text-xs md:text-sm leading-relaxed">
                                                                ...
                                                                @if($url = $getDownloadLink($syarat, $keterangan))
                                                                    <a href="{{ $url }}" target="_blank" rel="noopener noreferrer" aria-label="Unduh berkas {{ $syarat }} (Buka di tab baru)" class="ml-1.5 px-2 py-0.5 text-[10px] bg-blue-100 hover:bg-blue-200 text-blue-800 border border-blue-300 font-bold rounded hover:shadow-sm transition inline-flex items-center gap-0.5 focus:outline-none focus:ring-2 focus:ring-blue-600">
