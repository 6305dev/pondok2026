...
<div class="background-image-container min-h-screen pt-20 flex items-start justify-center font-sans">
    <div class="w-full max-w-sm bg-white/30 backdrop-blur-sm rounded-xl shadow-xl p-8 transform transition-transform duration-300">
        ...
        {{-- Form diarahkan ke route pengiriman WA --}}
        <form method="POST" action="{{ route('password.wa') }}" class="space-y-6">
            @csrf
            ...
            <div class="flex flex-col gap-4 mt-6">
                <button type="submit" class="w-full flex items-center justify-center bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-lg focus:outline-none focus:shadow-outline transition duration-200 shadow-md transform hover:scale-105">
